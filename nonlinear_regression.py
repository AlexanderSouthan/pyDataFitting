...
def nonlinear_regression(x_values, y_values, function_type, z_values=None,
    ...
    if alg == 'evo':
        return differential_evolution(
            ...
            args=(x_values, y_values, function_type, alg), maxiter=max_iter)
    if alg == 'lm':
        return least_squares(
            fit_error, initial_guess,
            args=(x_values, y_values, function_type, alg), method='lm')
...
def calc_function(x_values, parameters, function_type):
    function_names = ['polynomial', 'Gauss', 'rectangle_gauss_convolution',
                      'Langmuir', 'triangle']
    assert function_type in function_names, ('Unknown function type, allowed '
                                             'values are {}'.format(
                                                 function_names))
