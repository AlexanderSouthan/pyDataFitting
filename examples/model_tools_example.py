# -*- coding: utf-8 -*-
"""
Created on Fri Apr 14 09:24:14 2023
...
# some made up, very simple data for the ols fitting in a DataFrame with column
# names that are the factors and the response used later
x, y = np.meshgrid(np.linspace(1, 20, 5), np.linspace(1, 20, 5))
z = x + y
noise = np.random.normal(size=z.shape)
data = pd.DataFrame(np.array([x.ravel(), y.ravel(), (z + noise).ravel()]).T, columns=['A', 'B', 'Resp'])
...
for idx, (curr_x, curr_y) in enumerate(zip(x.ravel(), y.ravel())):
    z_fit[idx] = model_t.calc_model_value([curr_x, curr_y], fit_model.params)
